...
PreProcessingThreeDAlgorithm::PreProcessingThreeDAlgorithm() :
...
StatusCode PreProcessingThreeDAlgorithm::Reset()
{
...
StatusCode PreProcessingThreeDAlgorithm::Run()
{
    if (!this->GetPandora().GetSettings()->SingleHitTypeClusteringMode())
    {
        std::cout << "PreProcessingThreeDAlgorithm: expect Pandora to be configured in SingleHitTypeClusteringMode." << std::endl;
    ...
    catch (const StatusCodeException &statusCodeException)
    {
        ...
        {
            std::cout << "PreProcessingThreeDAlgorithm: Excessive number of hits in event, skipping the reconstruction" << std::endl;
    ...
    if (!m_currentCaloHitListReplacement.empty())
    {
        if (STATUS_CODE_SUCCESS != PandoraContentApi::ReplaceCurrentList<CaloHit>(*this, m_currentCaloHitListReplacement))
        {
            if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
                std::cout << "PreProcessingThreeDAlgorithm: could not replace current calo hit list with list named: " << m_currentCaloHitListReplacement
                          << std::endl;
        }
    }
...
void PreProcessingThreeDAlgorithm::ProcessCaloHits()
{
    ...
    CaloHitList selectedCaloHitListU, selectedCaloHitListV, selectedCaloHitListW, selectedCaloHitList3D;
    ...
    for (const CaloHit *const pCaloHit : *pCaloHitList)
    {
        ...
        {
            if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
                std::cout << "PreProcessingThreeDAlgorithm: found a hit with zero energy, will remove it" << std::endl;
        ...
        if ((pCaloHit->GetCellLengthScale() < m_minCellLengthScale) || (pCaloHit->GetCellLengthScale() > m_maxCellLengthScale))
        {
            if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
            {
                std::cout << "PreProcessingThreeDAlgorithm: found a hit with extent " << pCaloHit->GetCellLengthScale() << ", require ("
    ...
    if (!selectedCaloHitList3D.empty() && !m_outputCaloHitListName3D.empty())
        PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS, !=, PandoraContentApi::SaveList(*this, selectedCaloHitList3D, m_outputCaloHitListName3D));
}

//------------------------------------------------------------------------------------------------------------------------------------------

void PreProcessingThreeDAlgorithm::PopulateVoidCaloHitLists() noexcept
{
    ...
    {
        std::cout << "PreProcessingThreeDAlgorithm: Unable to create void calo hit lists" << std::endl;
    }
}

//------------------------------------------------------------------------------------------------------------------------------------------

void PreProcessingThreeDAlgorithm::GetFilteredCaloHitList(const CaloHitList &inputList, CaloHitList &outputList)
{
    HitKDTree2D kdTree;
...
StatusCode PreProcessingThreeDAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
{
