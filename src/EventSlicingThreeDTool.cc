...
EventSlicingThreeDTool::EventSlicingThreeDTool() :
...
void EventSlicingThreeDTool::RunSlicing(const Algorithm *const pAlgorithm, const HitTypeToNameMap &caloHitListNames,
...
void EventSlicingThreeDTool::CopyAllHitsToSingleSlice(const Algorithm *const pAlgorithm, const HitTypeToNameMap &caloHitListNames, Slice3DList &sliceList) const
{
    if (!sliceList.empty())
    ...
    PANDORA_THROW_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_INITIALIZED, !=,
        PandoraContentApi::GetList(*pAlgorithm, caloHitListNames.at(TPC_3D), pCaloHitList3D));

    if (pCaloHitListU || pCaloHitListV || pCaloHitListW || pCaloHitList3D)
    {
        sliceList.push_back(Slice3D());
        Slice3D &slice(sliceList.at(0));
        ...
        if (pCaloHitList3D)
            slice.m_caloHitList3D = *pCaloHitList3D;
    }
}

//------------------------------------------------------------------------------------------------------------------------------------------

void EventSlicingThreeDTool::GetThreeDClusters(
    const Algorithm *const pAlgorithm, const std::string &pfoListName, ClusterList &clusters3D, ClusterToPfoMap &clusterToPfoMap) const
{
    ...
    {
        if (PandoraContentApi::GetSettings(*pAlgorithm)->ShouldDisplayAlgorithmInfo())
            std::cout << "EventSlicingThreeDTool: unable to find pfo list " << pfoListName << std::endl;
...
void EventSlicingThreeDTool::GetClusterSliceList(const ClusterList &trackClusters3D, const ClusterList &showerClusters3D, ClusterSliceList &clusterSliceList) const
{
    ...
    for (const Cluster *const pCluster3D : trackClusters3D)
    {
        ...
        {
            std::cout << "EventSlicingThreeDTool: ThreeDSlidingFitResult failure for track cluster." << std::endl;
        }
    }
    ...
    for (const Cluster *const pCluster3D : showerClusters3D)
    {
        ...
        {
            std::cout << "EventSlicingThreeDTool: ThreeDSlidingConeFitResult failure for shower cluster." << std::endl;
        }
    }
...
void EventSlicingThreeDTool::CollectAssociatedClusters(const Cluster *const pClusterInSlice, const ClusterVector &candidateClusters,
...
bool EventSlicingThreeDTool::PassPointing(
...
bool EventSlicingThreeDTool::PassProximity(const Cluster *const pClusterInSlice, const Cluster *const pCandidateCluster) const
{
...
bool EventSlicingThreeDTool::PassShowerCone(
...
bool EventSlicingThreeDTool::CheckClosestApproach(const LArPointingCluster::Vertex &vertex1, const LArPointingCluster::Vertex &vertex2) const
{
...
bool EventSlicingThreeDTool::IsNode(const LArPointingCluster &cluster1, const LArPointingCluster &cluster2) const
{
...
bool EventSlicingThreeDTool::IsEmission(const LArPointingCluster &cluster1, const LArPointingCluster &cluster2) const
{
...
void EventSlicingThreeDTool::CreateSlices(const ClusterSliceList &clusterSliceList, Slice3DList &sliceList, ClusterToSliceIndexMap &clusterToSliceIndexMap) const
{
    ...
    Slice3DList &sliceList, ClusterSet &assignedClusters) const
{
    ...
    for (const Cluster *const pCluster3D : clusterList)
    {
        ...
        Slice3D &slice(sliceList.at(index));
...
void EventSlicingThreeDTool::GetRemainingClusters(const Algorithm *const pAlgorithm, const HitTypeToNameMap &clusterListNames,
...
void EventSlicingThreeDTool::GetRemainingClusters(const Algorithm *const pAlgorithm, const std::string &clusterListName,
    const ClusterSet &assignedClusters, ClusterList &remainingClusters) const
{
    ...
    {
        if (PandoraContentApi::GetSettings(*pAlgorithm)->ShouldDisplayAlgorithmInfo())
            std::cout << "EventSlicingThreeDTool: unable to find cluster list " << clusterListName << std::endl;
...
void EventSlicingThreeDTool::AssignRemainingHitsToSlices(
    const ClusterList &remainingClusters, const ClusterToSliceIndexMap &clusterToSliceIndexMap, Slice3DList &sliceList) const
{
    ...
    try
    {
        ...
        pointsU.sort(EventSlicingThreeDTool::SortPoints);
        pointsV.sort(EventSlicingThreeDTool::SortPoints);
        pointsW.sort(EventSlicingThreeDTool::SortPoints);
    ...
    {
        std::cout << "EventSlicingThreeDTool::AssignRemainingHitsToSlices - exception " << std::endl;
...
const EventSlicingThreeDTool::PointKDNode2D *EventSlicingThreeDTool::MatchClusterToSlice(const Cluster *const pCluster2D, PointKDTree2D &kdTree) const
{
    ...
    {
        std::cout << "EventSlicingThreeDTool::MatchClusterToSlice - exception " << std::endl;
...
bool EventSlicingThreeDTool::SortPoints(const CartesianVector *const pLhs, const CartesianVector *const pRhs)
{
...
StatusCode EventSlicingThreeDTool::ReadSettings(const TiXmlHandle xmlHandle)
{
